...
class McpServerMainTest {
    ...
    void serverNameConstantIsDefined() throws Exception {
        ...
        assertThat(McpServerMain.serverName()).isEqualTo(serverName);
    }
    ...
    void serverVersionConstantIsDefined() throws Exception {
        ...
        assertThat(McpServerMain.serverVersion()).isEqualTo(serverVersion);
    }

    @Test
    void serverVersionIsNotPinnedToLegacyConstant() throws Exception {
        Field serverVersionField = McpServerMain.class.getDeclaredField("SERVER_VERSION");
        serverVersionField.setAccessible(true);
        String serverVersion = (String) serverVersionField.get(null);

        assertThat(serverVersion).isNotEqualTo("1.0.0");
        assertThat(serverVersion).matches("\\d+\\.\\d+\\.\\d+.*");
    }
    ...
    void toolDescriptions_areInformative() {
        ...
        String[] descriptions = {
            ...
            "Get the current JDTLS indexing status. Returns 'ready' when indexing is complete, or progress details while still indexing.",
    ...
    void toolNamesMatchJavaToolsMethods() {
        ...
        assertThat(EXPECTED_TOOL_NAMES).contains("indexing_status");
